...
parser.add_argument('--sequential', action='store_true', help='Run ptraj and gbsa parts sequentially in a unique slurm job', required=False)
...
if args.last_part != args.first_part:
...
if args.sequential:
    ...
    run(f'sbatch --dependency=afterok:{jobid} ~/scripts/1cpu.sh ~/cdk_scripts/run/run_ptraj_gbsa.py -c {args.complex} -fp {args.first_part} -lp {args.last_part} -r {args.repeat} --test {args.test}', print_cmd=print_cmd)
